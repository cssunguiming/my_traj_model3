...
def predict_train_epoch(epoch, model, train_data, train_queue, optimizer, device, batch_size):
    ...
    for i, batch_queue in enumerate(train_queue):
        ...
        loss = model(input_loc, input_tim, max_history, pos=loc_label, neg=neg_label)
        ...
        if i % 100 == 0:
            try:
                print("{} epoch: {:_>2d} | iter: {:_>4d}/{:_>4d} | loss: {:<10.7f} | avg_loss: {:<10.7f} | lr: {:<9.7f} |".format(
                    desc, epoch, i, len_batch, iter_100_loss/100., avg_loss, optimizer._print_lr()))   
                iter_100_loss= 0
...
def predict_valid_epoch(epoch, model, valid_data, valid_queue, optimizer, device, batch_size):
    ...
    iter_100_loss, iter_100_loc, iter_100_cor_loc = 0, 0, 0
    total_loc, total_cor_loc = 0, 0
    eva_metric = np.zeros((6, 1))
    ...
    with torch.no_grad():
        for i, batch_queue in enumerate(valid_queue):
            ...
            place_logit = model.predict(input_loc, input_tim, max_history)

            loss, n_loc, n_cor_loc = cal_loss_performance(logit1=place_logit, label1=loc_label, Predict=True)
            eva_metric = get_acc(loc_label, place_logit, eva_metric)
...
def run(epoch, model, optimizer, device, train_data, train_traj_id, valid_data, valid_traj_id, log=None, batch_size=4):
    # run(Epoch, model, optimizer, device, train_data, train_traj_id, test_data, test_traj_id, log=log)
...
def main(Epoch=200, Bert_Pretrain=False, Batch_size=8, Pretrained=False, log='predict'):
    ...
    print("Get Freqs")
    freqs = get_freqs(dataset_4qs['vid_list'])
    print("User number: ", len(dataset_4qs['uid_list']))
    print("Generate Train_traj_list")
    train_data, train_traj_id = generate_input_history(data_neural=dataset_4qs["data_neural"], mode="train", freqs=freqs)
    print("Generate Valid_traj_list")
    test_data, test_traj_id = generate_input_history(data_neural=dataset_4qs["data_neural"], mode="test", freqs=freqs)
    ...
    run(Epoch, model, optimizer, device, train_data, train_traj_id, test_data, test_traj_id, log, Batch_size)
